...
- 支持的模型提供方：Qwen、DeepSeek、OpenAI、Gemini、MNN（本地端侧）
- 前端默认通过 `VITE_API_BASE_URL` 或本地存储决定后端地址，便于本地联调和部署切换

## 端侧推理（比赛版）

项目支持基于阿里 MNN 框架的 Qwen 模型本地推理，在 Android 设备上 100% 离线运行，无需任何云端 API 调用。

相比云 API 模式的优势：

- 完全离线：不依赖网络和云端服务，适合演示和受限环境
- 零费用：无需 API Key，无调用成本
- 隐私安全：所有数据和推理均在本地完成

当前限制：

- 仅支持 Android 平台（ARMv8.2+ 处理器）
- 端侧模型参数量较小，题目生成质量略低于云端大模型
- 每次出题数量建议控制在 3-5 题以内，材料长度建议在 800-1500 字

支持的硬件加速：

- ARMv8.2+ 基础指令集优化
- SME2 指令集加速（需较新 NDK，可选编译）

构建要求：

- Android NDK（r25+）
- MNN SDK（预编译库或源码编译）
- Qwen 模型文件（需通过 MNN llmexport.py 转换为 .mnn 格式）
- 模型文件和 MNN so 库放入 `frontend/nativeplugins/MnnQwenPlugin/android/libs/` 对应目录

在个人页加载本地模型后，即可使用演示材料体验端侧出题，或粘贴自己的学习材料开始练习。
